...
final class SEPWorker extends AtomicReference<SEPWorker.Work> implements Runnable {
    ...
    public void run() {
         ...
         *      this allows a producer to skip signalling work if the task queue is currently non-empty, and in
         *      conjunction
        ...
        try {
            while (true) {
                ...
                // if stop was signalled, go to sleep (don't try self-assign; being put to sleep is rare, so let's
                // obey it
     ...
     * -> (ASSIGNED)|SPINNING
     * STOP_SIGNALLED:  the worker has been asked to deschedule itself, but has not yet done so; only entered from a
     * SPINNING
     ...
     * (ASSIGNED):      asks the worker to perform some work against the specified executor, and preassigns a task
     * permit
